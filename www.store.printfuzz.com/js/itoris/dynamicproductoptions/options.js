...
DynamicProductOptions.prototype = {
	...
	initForm : function() {
		if (this.config.is_grouped) {
			...
			if (!this.groupProducts.length) {
				this.addToCartButton = this.productForm.select('.add-to-box .btn-cart')[0];
			...
			this.addToCartButton = this.productForm.select('.product-options-bottom .btn-cart')[0];
			if (!this.addToCartButton) {
				return;
			}
			this.addToCartButton.callback = this.addToCartButton.onclick;
			this.addToCartButton.onclick = null;
            this.addToCartButton.removeAttribute('onclick');
			this.addToCartButton.observe('click', this.showAddToCartPopup.bind(this));
            
            this.productForm._submit = this.productForm.submit;
			...
			if (this.getAppearance() == 'popup_cart' || this.getAppearance() == 'popup_configure') {
				...
				var configurableList = $$('.configurable-list, #product-options-wrapper .swatch-attr');
				for(var i=configurableList.length-1; i>=0; i--) $('itoris_dynamicproductoptions').insert({top: configurableList[i]});
                //additional check for configurable options
                var configurableList = optionsWrapper.select('dd select, dd input, dd textarea')[0];
                if (configurableList) configurableList = configurableList.up('dl');
                if (configurableList) $('itoris_dynamicproductoptions').insert({top: configurableList});
				...
				if (form) {
					...
					if (optionBottomBlock) {
						...
						var addToCart = optionBottomBlock.select('.add-to-cart')[0];
						var addToLinks = optionBottomBlock.select('.add-to-links')[0];
						if (addToCart) {
							this.addToCartBlock = addToCart;
							addToBox.appendChild(addToCart);
							if (!addToCart.select('.btn-cart')[0]) addToCart.appendChild(this.addToCartButton);
							if (this.getAppearance() == 'popup_configure') {
								addToCart.hide();
								var addToBoxConfigure = $('itoris_dynamicproductoptions_add_to_cart_configure');
								addToBoxConfigure.addClassName('add-to-cart');
								if (form.select('.product-shop .price-info')[0]) {
									form.select('.product-shop .price-info')[0].insert({after: addToBoxConfigure});
								} else {
				...
				if (optionsWrapper.select('dl').length > 1 || this.canShowWithoutOptions()) {
					...
					$('itoris_dynamicproductoptions').insert({top:optionsWrapper.select('dl')[0].addClassName('product-options')});
				}
				if (this.getAppearance() == 'popup_configure') {
					this.showConfiguration();
				}
		...
		if (this.popup) this.productForm.appendChild(this.popup);
	},
	...
	prepareOptions : function() {
					...
					if (this.options[i].type == 'radio' || this.options[i].type == 'checkbox') {
						for (var j = 0; j < this.options[i].items.length; j++) {
							if (typeof this.options[i].items[j] != 'undefined') {
								this.updateVisibility(this.options[i].items[j].visibility, this.options[i].items[j].elm.up('li'), this.options[i].items[j], false);
							}
						}
					}
		...
		if (typeof optionsPrice != 'undefined') {
			var obj = this;
			optionsPrice.addCustomPrices = function(key, priceObj) {
				...
				if (qtyElm) {
					...
					if (priceObj.price) {
						...
						if (tierPrice) {
							if (priceObj.price > 0) var k = tierPrice / priceObj.price; else var k = 1;
							priceObj = {
								price      : priceObj.price * k * qty,
								excludeTax : priceObj.excludeTax * k * qty,
								includeTax : priceObj.includeTax * k * qty,
								oldPrice   : priceObj.oldPrice * k * qty,
								priceValue : priceObj.priceValue * k * qty,
								type       : priceObj.type
							};
						} else {
							priceObj = {
								price      : priceObj.price * qty,
								excludeTax : priceObj.excludeTax * qty,
								includeTax : priceObj.includeTax * qty,
								oldPrice   : priceObj.oldPrice * qty,
								priceValue : priceObj.priceValue * qty,
								type       : priceObj.type
							};
						}
					}
				}
				this.customPrices[key] = priceObj;
			}.bind(optionsPrice);
		}
	...
	updateQtyComment: function(qtyElm) {
		...
		if (tierPrices.length > 0) {
			...
			for(var i=0; i<tierPrices.length; i++) {
				tierList += '<div>Buy <b>'+tierPrices[i].qty+'</b> for <b>'+formatCurrency(parseFloat(tierPrices[i].price), optionsPrice.priceFormat, false)+'</b> each</div>';
			}
	...
	getTierPrices: function(qtyElm) {
		...
		for(var i=0; i<qtyElm.parentElm.config.items.length; i++) {
			var tier = qtyElm.parentElm.config.items[i].tier_price, optionId = qtyElm.parentElm.value, optionConfigId = qtyElm.parentElm.config.items[i].item_count;
	...
	addQtyElement: function(elm) {
		...
		if ($(qtyId)) {
			...
			elmQty.value = 1;
			elmQty.parentElm = elm.config ? elm : elm.up('ul');
			elmQty.observe('change', function(){
				this.getOpConfig().reloadPrice();
			}.bind(this));
	...
	hideField: function(field) {
		if (this.canUseEffects) {
			if (field.visible()) {
				...
				new Effect.Parallel([
					new Effect.Fade(field, { sync: true}),
					new Effect.Morph(field, { sync: true, style: 'height:0;width:0;'})
				], {
					duration: 0.5,
	...
	showField: function(field) {
		if (this.canUseEffects) {
			if (!field.visible()) {
				...
				new Effect.Parallel([
					new Effect.Appear(field, { sync: true}),
					new Effect.Morph(field, { sync: true, style: 'height:' + targetHeight + 'px;width:' + targetWidth + 'px;'})
				], {
					duration: 0.5,
					afterFinishInternal: function(effect) {
						field.style.overflow = beforeOverflow;
						
						field.setStyle({width:'auto', height:'auto'});
				...
				if (typeof this.options[i].items != 'undefined') {
					...
					for (var j = 0; j < this.options[i].items.length; j++) {
						if (this.options[i].items[j].visibility_condition) {
							...
							if (conditions.conditions.length && this.isConditionCorrect(conditions)) {
								this.updateVisibility(this.options[i].items[j].visibility_action, isSelectElm ? this.options[i].items[j].elm : this.options[i].items[j].elm.up('li'), this.options[i].items[j], isSelectElm);
								if (this.options[i].items[j].visibility_action == 'visible') {
									isAnyVisible = true;
								}
							} else {
								this.updateVisibility(this.options[i].items[j].visibility, isSelectElm ? this.options[i].items[j].elm : this.options[i].items[j].elm.up('li'), this.options[i].items[j], isSelectElm);
					...
					if (this.options[i].type == 'drop_down' && !this.options[i].visibility_condition) {
						if (isAnyVisible) {
	...
	prepareRadioOptions : function(field, option) {
			...
			for (var j = 0; j < allRadios.length; j++) {
				if (allRadios[j].value == option.items[i].option_type_id) {
					...
					if (option.items[i].css_class) {
						allRadios[j].up('li').addClassName(option.items[i].css_class);
					}
					option.items[i].elm = allRadios[j];
					if (option.items[i].sku_is_product_id && !option.items[i].is_salable) {
						var elmLabel = allRadios[j].up('li').select('label')[0];
						if (elmLabel) {
		...
		if (carriageReturnElms.length) {
			for (var i = 0; i < carriageReturnElms.length; i++) {
				var nextElm = carriageReturnElms[i].up('li').next();
				if (nextElm) {
	...
	getOrigOptionField : function(option) {
		...
		switch (type) {
			...
			case 'file':
				...
				var deleteFileCheckbox = field.up('div.input-box').up().select('input[type=checkbox]')[0];
				if (deleteFileCheckbox) {
					...
					deleteFileIcon.observe('click', function(){
						...
						deleteFileIcon.next('.input-box').show();
						deleteFileIcon.up().select('input[type=file]')[0].disabled = false;
						deleteFileIcon.remove();
					});
				}
				field.up('div.input-box').up().childElements().each(function(elm){container.appendChild(elm)});
				return container;
			case 'drop_down':
			case 'multiple':
				field = $('select_' + id);
		...
		if (field) {
			...
			this.wrapField(field);
			return field.up('div.input-box');
		}
		return null;
	},
	wrapField: function(field) {
		if (!field.up('div.input-box') && field.up('dd')) {
			var box = field.up('dd');
			var childElements = box.childElements();
			var wrapElm = document.createElement('div');
			Element.extend(wrapElm);
			wrapElm.className = 'input-box';
	...
	scrollToValidationFailed: function(elm) {
		...
		this.getDynamicOptionsBlock().select('.options-list.validation-failed').each(function(elm) {
	...
	showPopup : function() {
		...
		var beforeStyle = 'height:' + this.popup.getHeight() + 'px;width:' + this.popup.getWidth() + 'px;margin-left:-' + (maxWidth / 2) + 'px;top:' + (topScrollOffset + 75) + 'px;';
	...
	resizePopup: function() {
		...
		var newStyles = 'width:' + newWidth + 'px;margin-left:-' + (maxWidth / 2) + 'px;top:' + (topScrollOffset + 75) + 'px;';
		this.popup.setStyle({width:beforeWidth + 'px'});
		new Effect.Morph(this.popup, {
			style: newStyles,
			duration: 0.05,
	...
	showAddToCartPopup : function(ev) {
		...
		if (this.addToCartButton.onclick) {
            var _this = this;
            if (this.getAppearance() == 'popup_cart') {
                //prevent form from submitting twice due to the MRSP when popup appears after clicking "add to cart" 
                this.productForm.submit = function(ev){
                    _this.productForm._submit(ev);
                    _this.productForm.submit = null;
                };
            }
			...
			setTimeout(function(){
				...
				if ($$('.validation-failed')[0]) {
					this.scrollToValidationFailed($$('.validation-failed')[0]);
				}
			}.bind(this), 100);
		} else {
			if (this.getAppearance() == 'popup_cart') {
	...
	addToCartFromPopup : function(ev) {
		this.addToCartButton.onclick = this.addToCartButton.callback;
		this.productForm.appendChild(this.popup);
		this.hideDefaultMessages();
		this.removeItorisValidationAdvices();
	...
	addToCartFromPopupGrouped : function() {
		...
		this.groupProducts.each(function(obj){
			...
			if (obj.getAppearance() == 'popup_configure' && obj.getGroupProductQty()) {
				obj.popup.hide();
				if (obj.popup.select('.validation-failed')[0]) {
					obj.addGroupedPopupValidationAdvice();
				}
			}
		});
	...
	applyPopupConfig : function() {
		...
		if (formValidation.validate()) {
			...
			this.showDefaultMessages();
			$$('body')[0].appendChild(this.popup);
			if (this.popup.select('.validation-failed')[0]) {
				this.scrollToValidationFailed(this.popup.select('.validation-failed')[0]);
			} else if (this.isGrouped() && this.productForm.select('.validation-failed')[0]) {
				this.hidePopup();
				this.showConfiguration();
			}
		}
	...
	showConfiguration : function() {
		if (this.isGrouped()) {
			...
			if (this.addToCartBlock.up('.product-shop') && this.addToCartBlock.up('.product-shop').select('.price-info')[0]) {
				this.addToCartBlock.up('.product-shop').select('.price-info')[0].insert({after:$('itoris_dynamicproductoptions_configuration')});
			} else {
				this.addToCartBlock.up().insert({before: $('itoris_dynamicproductoptions_configuration')});
			}
		...
		for (var i = 0; i < labels.length; i++) {
			if (values[i]) {
				try {
					...
					if (values[i].select('.input-box select')[0]) {
						var selectBox = values[i].select('.input-box select')[0];
					...
					} else if (values[i].select('.input-box input[type=radio]')[0]) {
						var radios = values[i].select('.input-box input[type=radio]');
					...
					} else if (values[i].select('.input-box input[type=checkbox]')[0]) {
						var checkboxes = values[i].select('.input-box input[type=checkbox]');
						value = [];
						...
						value = this.simpleStripTags(values[i].select('.input-box')[0].innerHTML);
					}
	...
	showFieldError : function(optionId, message) {
		...
		if (option) {
			...
			switch (option.type) {
				...
				case 'time':
				default:
					errorElm = fieldBox.select('.input-box')[0];
			}
			if (errorElm) {
				errorElm.addClassName('validation-failed');
...
Calendar.prototype.showAt = function (x, y) {
	...
	x = viewOffset.left + document.viewport.getScrollOffsets().left - 225;
	y = viewOffset.top + document.viewport.getScrollOffsets().top + 15;
